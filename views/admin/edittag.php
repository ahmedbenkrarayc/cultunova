...
$tag = new Tag($_GET['id'], null);
$current = $tag->tagById();

if(count($tag->getErrors()) > 0 || $current == null){
    header('Location: ./tags.php');
}
            ...
            <form action="<?php echo $_SERVER['PHP_SELF'].'?id='.$current['id'] ?>" method="POST" class="card" id="form">
                ...
                <div class="card-body">
                  <div class="mb-3">
                    ...
                    <div>
                      <input type="text" class="form-control" placeholder="Enter tag" name="name" id="name" value="<?php echo $current['name'] ?>">
                    </div>
                  </div>
                </div>
